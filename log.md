...
- **Big Data Analysis 12th Written Exam Comprehensive Scorecard & Gap Analysis Report Created**: 수험자의 실제 채점 데이터(49/80, 61.25점)를 기반으로 과목별 정밀 진단, 세부 영역별 취약점 역공학 해부 및 합격 안정권(+6문제) 달성 로드맵을 수록한 [제12회 기출 성적 진단 및 취약점 역공학 보고서](wiki/ml/bigdata_analysis/past-exam-12-analysis.md)를 생성하고 인덱스에 연결했다.
- **Big Data Analysis 12th Written Exam Review 100% Complete (All 80 Questions Ingested)**: 2026년 상반기 시행 제12회 빅데이터 분석기사 필기 기출문제 제4과목 빅데이터 결과 해석(61번~80번, 20문항) 및 전 과목 80문항 전체 복기와 오답노트 작성을 완벽하게 완료했다.
  - **제4과목 주요 복기 내용**: 데이터 누수(Data Leakage, Split First Fit Later), 혼동행렬 기반 평가지표(Precision, Recall, F1-Score 조화평균 및 $FP=FN$ 3초 컷), Grid Search CV 총 학습 횟수 계산 공식(($5\times10)\times5 = 250$회), 데이터 불균형(Class Imbalance, 정확도의 착시, SMOTE 보간 합성 vs 언더샘플링 정보 손실), ROC 곡선 축($x$: FPR, $y$: TPR, 좌상단 $(0, 1)$ 최적) 및 AUC(1.0 최우수, 0.5 무작위 추측), 지도학습 유형별 평가 지표(회귀: MSE/RMSE/MAE/R² vs 분류: Accuracy/F1/AUC), 하이퍼파라미터 4대 탐색(수동 최적 미보장, 그리드 전수 탐색, 랜덤 고차원 효율, 베이지안 사후확률), 홀드아웃(1회, 분할 편향) vs K-Fold CV(K회 학습, 계산비용/시간 증가, 평가 신뢰도 극대화), 분석 모델 리모델링 4단계(데이터 $\to$ 피처 $\to$ 튜닝 $\to$ 과적합 재검증, 보안 규정은 거버넌스), 정량적 평가(수치 지표 객관적 비교) vs 정성적 평가(활용성, 용이성, 적합성), 모델 전개(Deployment, 실제 업무 시스템 적용 및 성능 모니터링/유지보수 계획), 메타 앙상블(스태킹 K-Fold OOF vs 블렌딩 Hold-out), 연관 분석(장바구니 숨겨진 패턴 발견, 고정된 속담 적용 불가), 신뢰도(Confidence, 분모=화살표 왼쪽) 및 향상도(Lift, 대칭성) 계산, Box-Cox 변환(양수 $x>0$ 필수) 및 Shapiro-Wilk 정규성 검정($p \ge 0.05$여야 정규성 만족), 포아송 분포(평균=분산=$\lambda$) 및 카이제곱 적합도 통계량($\chi^2$ 작을수록 분포에 적합), 인포그래픽(시각적 스토리텔링 미디어, 가설검정/성능산출 알고리즘 아님), 비교 시각화 5대 기법(산점도 점 vs 레이더 차트 방사형 다각형 꼭짓점, 히트맵, 평행좌표, 체르노프 페이스), 히스토그램(연속 수치 Bin, 막대 붙음, 순서 고정) vs 막대그래프(범주, 막대 분리, 순서 임의 변경 가능), 시각화 5대 목적별 유형(공간 시각화: 카토그램, 등치선도, 도트맵, 단계구분도)을 [제12회 기출문제 복기 노트](wiki/ml/bigdata_analysis/past-exam-12-review.md)에 집대성 완료했다.
...
- **Big Data Analysis 12th Written Exam Review (Subject 2 Complete)**: 2026년 상반기 시행 제12회 빅데이터 분석기사 필기 기출문제 제2과목 빅데이터 탐색(21번~40번, 20문항) 전체 복기 및 오답노트 작성을 완료했다.
  - **주요 복기 내용**: 중심적률 4대 차수(2분 3왜 4첨), 확률표본추출 4총사(단순/계통/층화/군집), 이상값 발생 원인(측관보 vs 표본오차), 독립확률변수 합의 분산/표준편차($\sqrt{2}\sigma$), 이산확률변수 분산(제평-평제, 주사위 $35/12$), 피어슨 상관계수(선형 vs 비선형 한계), 중심극한정리(CLT) vs 대수의 법칙(LLN), 상자그림(Box Plot) 5수치 및 IQR 이상치 경계, 차원 축소 5대 기법(SVD 직사각형 분해/PCA/LDA/ICA/FA), 범주형 원-핫 인코딩(직교성 및 의미적 유사성 한계), 가설검정 오류 체계(제1종 오류 알파/유의수준 vs 제2종 오류 베타), 데이터 변환 4대 기법(평활화 잡음제거 vs 특성생성), 가설검정 5단계 절차(가유통피결), 변수 선택 3총사(필터/래퍼/임베디드), 대푯값 4대 평균(산술/절사/기하/조화), 차원 축소의 정보 손실 필연성, 산포도 4대 지표 단위 체계(분산 단위제곱, CV 무차원), 결측값 처리 3기법 및 결측 메커니즘(MCAR/MAR/MNAR), PCA(전체분산) vs 요인분석(공통분산), PCA 제1주성분 축 방향($A-B$ 및 단위벡터)을 [제12회 기출문제 복기 노트](wiki/ml/bigdata_analysis/past-exam-12-review.md)에 수록 완료했다.
- **Daily Routine Synchronization (2026-08-25 ~ 2026-09-01)**: 2026년 8월 25일부터 9월 1일까지의 일일 루틴 브리핑(미국 경제, AI Paper & Tech, 영어 학습, LLM/ML 채용 공고) 및 체크리스트를 전수 검증하고 동기화 완료했다. 주말 영어 휴식 규칙 및 공고 유무에 따른 체크리스트 상태를 1:1로 정밀 일치시켰다.
...
- **EMBER2024 Classifier Gate**: train 26,000건을 weeks 0–43 fit·44–51 calibration으로 나누고 test weeks 52–63의 6,000건을 평가했다. 자체 temporal LightGBM은 precision/recall `0.9793/0.9293`이지만 FPR `0.0197`, 주별 최대 FPR `0.056`으로 실패했다. 공식 공개 모델도 같은 calibration 계약에서 FPR `0.1097`, 주별 최대 FPR `0.208`로 실패했다.
- **Decision**: [AegisLM EMBER2024 ELF Classifier 절대평가](wiki/projects/Fine_Tuned/training/aegislm_ember2024_classifier_baseline_decision_20260731.md)를 추가했다. 두 모델의 NuriLab static-signal 연결과 Qwen SFT 혼합을 보류하고 FP 집중 주차의 feature drift 감사를 다음 gate로 고정했다.
- **EMBER2024 Benchmark Gate**: 고정 ELF test archive의 12개 weekly JSONL·12,000행을 비실행 감사했다. 동일 `(week_id, sha256)` 중복 6,000행을 확인했고 primary label·static feature 충돌은 0건이었다. CAPS/MBC/TTP 보조 annotation 차이는 별도 merge 계약 전까지 사용하지 않으며, 6,000 temporal observation으로 중복 제거한 독립 malware benchmark만 승인했다.
- **Decision**: [AegisLM EMBER2024 ELF Static-Feature Benchmark Gate](wiki/projects/Fine_Tuned/training/aegislm_ember2024_benchmark_decision_20260731.md)를 추가했다. Qwen SFT와 raw executable download는 승인하지 않고, 다음 단계는 deterministic materializer와 별도 malware classifier absolute gate로 고정했다.
- **BinKit Metadata Gate**: v2.0.0의 8 architectures·23 compilers·6 optimization levels·371,928 binaries matrix는 확인했지만 GitHub release dataset asset이 0개이고 Drive artifact의 size·SHA-256·dataset license·row schema가 없어 `metadata_hold`로 판정했다. binary와 함수 pickle은 다운로드·역직렬화하지 않았다.
- **Decision**: [AegisLM BinKit 2.0 Metadata Gate](wiki/projects/Fine_Tuned/training/aegislm_binkit_metadata_decision_20260731.md)를 추가하고 다음 후보를 EMBER2024 독립 malware-feature benchmark gate로 고정했다.
- **Assemblage Metadata Gate**: 고정 LinuxELF DuckDB artifact 21.97GB의 size·SHA-256·zstd 무결성과 136.14GB read-only schema를 검증했다. 249,121행 중 repository·optimization은 완전했지만 actionable license 상한 70.91%, architecture 68.84%, format·commit·build mode 약 31.15%였고 strict complete row는 0건이었다. raw ELF archive는 받지 않고 `metadata_reference_only / training=false`로 종료했다.
- **Decision**: [AegisLM Assemblage LinuxELF Metadata Gate](wiki/projects/Fine_Tuned/training/aegislm_assemblage_metadata_decision_20260731.md)를 추가하고 다음 후보를 BinKit 2.0 metadata-only gate로 고정했다.
- **Decompile-Bench Gate**: 고정 Arrow shard 131,359건을 비실행 감사하고 seed `20260731`의 100건 source–assembly 정렬을 수동 검토했다. 96건 PASS, 다른 함수가 짝지어진 오류 4건으로 정렬 gate는 통과했지만 명시 repository 복원은 84.6581%였고 repository별 license·compiler·optimization metadata가 없어 `alignment_reference_only / training=false`로 판정했다.
- **Decision**: [AegisLM Decompile-Bench Alignment·Provenance Gate](wiki/projects/Fine_Tuned/training/aegislm_decompile_bench_alignment_decision_20260731.md)를 추가했다. 다음 단계는 전체 실행파일 corpus가 아닌 Assemblage metadata-only 감사로 고정했다.
- **CVEfixes Manual Gate**: actionable CWE 200쌍 review queue를 고정 순서로 검토해 28건 중 오류·불확실 11건에서 예산 10건을 초과했다. 남은 172건은 미검토로 보존하고 `manual_review_fail_early`로 종료했다. broad/mismatched CWE, 함수 안에 target operation이 없는 간접 변경, CWE와 무관한 함수 pair가 주된 실패 원인이며 CVEfixes commit-level CWE direct-label 학습과 repository license gate를 중단했다.
